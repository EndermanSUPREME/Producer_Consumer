...
void consumer(SharedData* data) {
    while (true) {
        sem_wait(&data->full);  // wait till the buffer is full
        sem_wait(&data->mutex); // wait for access to the buffer
        ...
        std::cout << "[*] Consumed: " << item_consumed << std::endl;

        sem_post(&data->mutex); // release the buffer
        sem_post(&data->empty); // send a signal when the buffer is empty and on element consumption
